...
```js
httpService.get("url", requestConfig);
httpService.head("url", requestConfig);
httpService.options("url", requestConfig);
httpService.delete("url", requestConfig);
httpService.put("url", data, requestConfig);
httpService.patch("url", data, requestConfig);
httpService.post("url", data, requestConfig);
httpService.request(config);
```

### Creating A New Instance

There is a ability to create a new instance with a new config rather than using old configurations.

```js
let httpClient = this.httpService.create(config);
```
...
```js
{
  ...
  // It can be convenient to set `baseURL` to pass relative URLs
...
The response comes back as a Promise so you can successful responses and handle errors.
