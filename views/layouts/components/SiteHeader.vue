<template>
   <header class="header">
       <nav class="site-header">
           ...
           <div class="site-header__mobile nav-button" :class="{ active : isMenuOpen }" @click="toggleMenu">
            <span>
                <i class="fal fa-bars fa-2x"></i>
            </span>
           </div>
           <div class="nav-dropdown" @click="showVersionDropDown = !showVersionDropDown" v-show="docsPage && versions.length > 1">
...
export default {
  ...
  methods: {
    selectVersion(version) {
      this.$router.push({ name: "docs", params: { version: version, page : 'what-is-varie' } });
    },
    toggleMenu() {
      if (!this.isMenuOpen) {
        this.$nextTick(() => {
